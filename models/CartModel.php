	...
	trait CartModel{		
		...
		public function cartCheckOut(){
			...
			foreach($_SESSION["cart"] as $product){
				...
				// $query1 = $conn->prepare("update products set quantity=:var_quantity where id=:var_id");
				// $query1->execute(array("order_id"=>$order_id,"product_id"=>$product["id"],"price"=>$product["price"],"quantity"=>$product["number"]));
			}
			//xoa gio hang
			unset($_SESSION["cart"]);
		}
